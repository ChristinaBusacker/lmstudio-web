...
import type { Edge, Graph } from './worker/workflow-worker.graph';
import { buildDependencies, topoSort } from './worker/workflow-worker.graph';
import {
  buildMessages,
  portIndex,
  renderTemplate,
  safeJsonParse,
  toText,
} from './worker/workflow-worker.template';
...
export class WorkflowWorkerService implements OnModuleInit, OnModuleDestroy {
  ...
  // (Graph + templating helpers were extracted to ./worker/* to keep this service readable.)
  ...
  private async executeSingleNode(args: {
    ...
    if (nodeType === 'workflow.merge') {
      ...
      edges.sort((a, b) => {
        const ai = portIndex(a.targetPort);
        const bi = portIndex(b.targetPort);
      ...
      for (const src of sources) {
        if (!(src in ctx.nodes))
          throw new Error(`Missing upstream output: ${src} required by ${nodeId}`);
        parts.push(toText(ctx.nodes[src]));
      }
    ...
    if (nodeType === 'workflow.export') {
      ...
      const text = toText(ctx.nodes[src]);
    ...
    if (nodeType === 'workflow.condition') {
      ...
      if (sourcesSorted.length > 0) {
        const inputText = toText(ctx.input);
        blocks.push(
      ...
      const gen = this.engine.streamChat(
        ...
        buildMessages(systemPrompt, finalPrompt),
        params,
      );
    ...
    const gen = this.engine.streamChat(
      `${runId}:${nodeId}:${iteration}`,
      buildMessages(systemPrompt, renderedPrompt),
      params,
    );
      ...
      for (const nodeId of nodeOrder) {
        ...
        if (nodeType === LOOP_START) {
          ...
          while (index < maxIterations) {
            ...
            const produced = lastBodyId ? toText(ctx.nodes[lastBodyId]) : '';
            items.push(produced);
